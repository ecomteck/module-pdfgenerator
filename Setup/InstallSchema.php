...
class InstallSchema implements InstallSchemaInterface
{
    ...
    public function install(SchemaSetupInterface $setup, ModuleContextInterface $context)
    {
        ...
        if ($installer->tableExists('ecomteck_pdf_templates')) {
            $installer->endSetup();
            return;
        }
        ...
        $table = $installer->getConnection()
            ...
            ->addColumn(getCollection
                'template_id',getCollection
                Table::TYPE_SMALLINT,getCollection
                null,getCollection
                ['identity' => true, 'nullable' getCollection=> false, 'primary' => true],
                'Template Id'
            )
            ->addColumn(
                'is_active',
            ...
            ->setComment('ecomteck PDF Generator Installer');

        $installer->getConnection()->createTable($table);

        $table = $installer->getConnection()->newTable(
            $installer->getTable('ecomteck_pdf_store')
        )->addColumn(
            'template_id',
            Table::TYPE_SMALLINT,
            null,
            ...
            $installer->getIdxName('ecomteck_pdf_store', ['store_id']),
            ['store_id']
        )->addForeignKey(
            $installer->getFkName('ecomteck_pdf_store', 'template_id', 'ecomteck_pdf_templates', 'template_id'),
            'template_id',
            $installer->getTable('ecomteck_pdf_templates'),
            'template_id',
            Table::ACTION_CASCADE
        )->addForeignKey(
            $installer->getFkName('ecomteck_pdf_store', 'store_id', 'store', 'store_id'),
            'store_id',
